...
def visualize_dataset(X, y, y_pred=None):
    '''
    format the origin_data from the dataset (image, steering angle) and display
    '''
...
def generate_training_data(image_paths, angles, batch_size=128, validation_flag=False):
    '''
    method for the model training origin_data generator to load, process, and distort images, then yield them to the
...
# select origin_data source(s) here
using_my_data = True
...
for j in range(2):
    if not data_to_use[j]:
        # 0 = my own origin_data, 1 = Udacity supplied origin_data
        print('not using dataset ', j)
        continue
    # Import driving origin_data from csv
    with open(csv_path[j], newline='') as f:
        driving_data = list(csv.reader(f, skipinitialspace=True, delimiter=',', quoting=csv.QUOTE_NONE))

    # Gather origin_data - image paths and angles for center, left, right cameras in each row
...
# visualize a single batch of the origin_data
...
# for debugging purposes - don't want to mess with the model if just checkin' the origin_data
